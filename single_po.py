...
def prefill(
    ...
    for e in prompt.elements:
        if isinstance(e, str):
            ...
            for ce in e.elements:
                print(ce)
                if isinstance(ce, Redacted):
                    ###typecast: Redacted
                    token_ids = tokenizer.encode(ce.text, add_special_tokens=False)
                    size = len(token_ids)
                    private_token_ids.extend(token_ids)
                    private_pos_ids.extend(range(pos_offset, pos_offset + size))
                    mask_info.append((1, size))  # local (gt)
                    pos_offset += size
    ...
    print('public buffer size', tokenizer.decode(public_token_ids))
    print('private buffer size', tokenizer.decode(private_token_ids))
