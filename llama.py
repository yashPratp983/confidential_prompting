...
class ConfidentialLlamaAttention(LlamaAttention):
    """Attention module using shared memory for confidential computations"""
    ...
    def set_shared_memory(self, shared_mem_manager, device):
        """Set the shared memory manager after initialization"""
    ...
    def forward(
        ...
        if confidential and self.shared_mem_manager is not None:
            # Use shared memory for confidential communication
            batch_per_user = bsz // num_users
            
            for i in range(num_users):
                # print("user-id",i)
                # Get the shared memory tensor and copy data to it
                q_tensor = self.shared_mem_manager.get_q_tensor_master(i, None)
                # print(q_new.shape)
                q_tensor.copy_(q_new_cpu[i * batch_per_user:(i + 1) * batch_per_user].cpu().half())
                
                # Signal that the Q tensor is ready
                self.shared_mem_manager.signal_q_ready(i)
                # print("q is signalled ready")
        ...
        k_new = apply_rotary_pos_emb1(k_new, cos, sin, position_ids)

        # Compute the public states synchronously
        ...
        max_pub, sum_pub, score_pub = softmax(score_pub, dim=-1)
        
        attn_pub = torch.matmul(score_pub.to(q_new.dtype), v_pub)
        logger.log_measure('pub_attention')
        
        if confidential and self.shared_mem_manager is not None:
            # Wait for workers to compute private attention
            o_tensors = []
            for i in range(num_users):
                if not self.shared_mem_manager.wait_for_o_ready(i):
                    return torch.zeros_like(hidden_states)  # Return early if termination signal received
                
                # Get the output tensor from shared memory
                o_tensor = self.shared_mem_manager.get_o_tensor_master(i, self.device)
                o_tensors=o_tensor[layer_id]
            
            # Combine outputs from all workers
            self.pvt_buffer_gpu = o_tensors
